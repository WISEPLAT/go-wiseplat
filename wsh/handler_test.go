...
func testGetNodeData(t *testing.T, protocol int) {
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*wshdb.MemDatabase).Keys() {
	...
	statedb, _ := wshdb.NewMemDatabase()
...
// Tests that post wsh protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", wsh63, pm, true)
	defer peer.close()
