...
type fetchRequest struct {
	...
	From    uint64              // [wsh/62] Requested chain element index (used for skeleton fills only)
	Hashes  map[common.Hash]int // [wsh/61] Requested hashes with their insertion index (priority)
	Headers []*types.Header     // [wsh/62] Requested headers, sorted by request order
...
type queue struct {
	...
	headerHead common.Hash // [wsh/62] Hash of the last queued header to verify order

	// Headers are "special", they download in batches, supported by a skeleton chain
	headerTaskPool  map[uint64]*types.Header       // [wsh/62] Pending header retrieval tasks, mapping starting indexes to skeleton headers
	headerTaskQueue *prque.Prque                   // [wsh/62] Priority queue of the skeleton indexes to fetch the filling headers for
	headerPeerMiss  map[string]map[uint64]struct{} // [wsh/62] Set of per-peer header batches known to be unavailable
	headerPendPool  map[string]*fetchRequest       // [wsh/62] Currently pending header retrieval operations
	headerResults   []*types.Header                // [wsh/62] Result cache accumulating the completed headers
	headerProced    int                            // [wsh/62] Number of headers already processed from the results
	headerOffset    uint64                         // [wsh/62] Number of the first header in the result cache
	headerContCh    chan bool                      // [wsh/62] Channel to notify when header download finishes

	// All data retrievals below are based on an already assembles header chain
	blockTaskPool  map[common.Hash]*types.Header // [wsh/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [wsh/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [wsh/62] Currently pending block (body) retrieval operations
	blockDonePool  map[common.Hash]struct{}      // [wsh/62] Set of the completed block (body) fetches

	receiptTaskPool  map[common.Hash]*types.Header // [wsh/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [wsh/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [wsh/63] Currently pending receipt retrieval operations
	receiptDonePool  map[common.Hash]struct{}      // [wsh/63] Set of the completed receipt fetches
