...
import (
	...
	wiseplat "github.com/wiseplat/go-wiseplat"
...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [wsh/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [wsh/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [wsh/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [wsh/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [wsh/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [wsh/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [wsh/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [wsh/63] Channel receiving inbound node state data
...
func New(mode SyncMode, stateDb wshdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
// these are zero.
func (d *Downloader) Progress() wiseplat.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "wsh", p.version, "head", hash, "td", td, "mode", d.mode)
