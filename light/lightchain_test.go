...
// header only chain.
func newCanonical(n int) (wshdb.Database, *LightChain, error) {
	db, _ := wshdb.NewMemDatabase()
	gspec := core.Genesis{Config: params.TestChainConfig}
	genesis := gspec.MustCommit(db)
	blockchain, _ := NewLightChain(&dummyOdr{db: db}, gspec.Config, wshash.NewFaker())
...
func newTestLightChain() *LightChain {
	...
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, wshash.NewFullFaker())
	if err != nil {
		panic(err)
	}
	return lc
...
func (odr *dummyOdr) Database() wshdb.Database {
	return odr.db
}
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestChainConfig, wshash.NewFaker())
	if err != nil {
