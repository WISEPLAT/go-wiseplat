...
type testOdr struct {
	OdrBackend
	sdb, ldb wshdb.Database
	disable  bool
}

func (odr *testOdr) Database() wshdb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db wshdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes1(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db wshdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db wshdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrAccounts(ctx context.Context, db wshdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db wshdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func testChainGen(i int, block *core.BlockGen) {
	...
	switch i {
	case 0:
		// In block 1, the test bank sends account #1 some wise.
		...
		block.AddTx(tx)
	case 1:
		// In block 2, the test bank sends some more wise to account #1.
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	var (
		sdb, _  = wshdb.NewMemDatabase()
		ldb, _  = wshdb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(sdb)
	)
	gspec.MustCommit(ldb)
	// Assemble the test environment
	blockchain, _ := core.NewBlockChain(sdb, params.TestChainConfig, wshash.NewFullFaker(), vm.Config{})
	...
	lightchain, err := NewLightChain(odr, params.TestChainConfig, wshash.NewFullFaker())
	if err != nil {
		t.Fatal(err)
	}
