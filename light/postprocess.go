...
import (
	...
	"github.com/wiseplat/go-wiseplat/common/bitutil"
...
func GetChtRoot(db wshdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func GetChtV2Root(db wshdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	return GetChtRoot(db, (sectionIdx+1)*(ChtFrequency/ChtV1Frequency)-1, sectionHead)
}

// StoreChtRoot writes the CHT root assoctiated to the given section into the database
// Note that sectionIdx is specified according to LES/1 CHT section size
func StoreChtRoot(db wshdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewChtIndexer(db wshdb.Database, clientMode bool) *core.ChainIndexer {
	cdb := wshdb.NewTable(db, ChtTablePrefix)
	idb := wshdb.NewTable(db, "chtIndex-")
...
const (
	...
	wshBloomBitsSection       = 4096
	wshBloomBitsConfirmations = 256
)
...
func GetBloomTrieRoot(db wshdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db wshdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db wshdb.Database, clientMode bool) *core.ChainIndexer {
	cdb := wshdb.NewTable(db, BloomTrieTablePrefix)
	idb := wshdb.NewTable(db, "bltIndex-")
	...
	if clientMode {
		...
		backend.parentSectionSize = wshBloomBitsSection
		confirmReq = HelperTrieProcessConfirmations
	}
	backend.bloomTrieRatio = BloomTrieFrequency / backend.parentSectionSize
	backend.sectionHeads = make([]common.Hash, backend.bloomTrieRatio)
	return core.NewChainIndexer(db, idb, backend, BloomTrieFrequency, confirmReq-wshBloomBitsConfirmations, time.Millisecond*100, "bloomtrie")
}
