...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, wsh Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		engine:         engine,
		wsh:            wsh,
		...
		chain:          wsh.BlockChain(),
		proc:           wsh.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(wsh.BlockChain(), miningLogAtDepth),
	}
	// Subscribe TxPreEvent for tx pool
	worker.txSub = wsh.TxPool().SubscribeTxPreEvent(worker.txCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = wsh.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = wsh.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
	go worker.update()
...
func (self *worker) setWisebase(addr common.Address) {
	self.mu.Lock()
...
func (self *worker) commitNewWork() {
	...
	pending, err := self.wsh.TxPool().Pending()
	if err != nil {
