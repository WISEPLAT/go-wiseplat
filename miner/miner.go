...
// Package miner implements Wiseplat block creation and mining.
package miner
...
func New(wsh Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		wsh:      wsh,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, wsh, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(wsh.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setWisebase(coinbase)
...
func (self *Miner) SetWisebase(addr common.Address) {
	self.coinbase = addr
	self.worker.setWisebase(addr)
}
