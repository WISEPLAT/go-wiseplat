...
// genTxRing returns a block generator that sends wise in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db wshdb.Database
	if !disk {
		db, _ = wshdb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "wsh-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = wshdb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
	...
	chainman, _ := NewBlockChain(db, gspec.Config, wshash.NewFaker(), vm.Config{})
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db wshdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "wsh-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		db, err := wshdb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, params.TestChainConfig, wshash.NewFaker(), vm.Config{})
		if err != nil {
