...
func (b *BlockGen) OffsetTime(seconds int64) {
	...
	b.header.Difficulty = wshash.CalcDifficulty(b.config, b.header.Time.Uint64(), b.parent.Header())
}
...
func GenerateChain(config *params.ChainConfig, parent *types.Block, db wshdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
	...
	genblock := func(i int, h *types.Header, statedb *state.StateDB) (*types.Block, types.Receipts) {
		...
		wshash.AccumulateRewards(config, statedb, h, b.uncles)
...
func makeHeader(config *params.ChainConfig, parent *types.Block, state *state.StateDB) *types.Header {
	...
	return &types.Header{
		...
		Difficulty: wshash.CalcDifficulty(config, time.Uint64(), &types.Header{
...
// header only chain.
func newCanonical(n int, full bool) (wshdb.Database, *BlockChain, error) {
	// Initialize a fresh chain with only a genesis block
	gspec := new(Genesis)
	db, _ := wshdb.NewMemDatabase()
	genesis := gspec.MustCommit(db)

	blockchain, _ := NewBlockChain(db, params.AllWshashProtocolChanges, wshash.NewFaker(), vm.Config{})
...
func makeHeaderChain(parent *types.Header, n int, db wshdb.Database, seed int) []*types.Header {
...
func makeBlockChain(parent *types.Block, n int, db wshdb.Database, seed int) []*types.Block {
