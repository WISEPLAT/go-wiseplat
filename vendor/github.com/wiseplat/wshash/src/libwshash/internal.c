...
bool wshash_compute_full_data(
	void* mem,
	uint64_t full_size,
	wshash_light_t const light,
	wshash_callback_t callback
)
{
	...
	for (uint32_t n = 0; n != max_n; ++n) {
		...
		wshash_calculate_dag_item(&(full_nodes[n]), n, light);
	}
	return true;
}
...
void wshash_quick_hash(
	wshash_h256_t* return_hash,
	wshash_h256_t const* header_hash,
	uint64_t nonce,
	wshash_h256_t const* mix_hash
)
{
	uint8_t buf[64 + 32];
...
bool wshash_quick_check_difficulty(
	wshash_h256_t const* header_hash,
	uint64_t const nonce,
	wshash_h256_t const* mix_hash,
	wshash_h256_t const* boundary
)
{

	wshash_h256_t return_hash;
	wshash_quick_hash(&return_hash, header_hash, nonce, mix_hash);
	return wshash_check_difficulty(&return_hash, boundary);
}
...
static bool wshash_mmap(struct wshash_full* ret, FILE* f)
{
	int fd;
	char* mmapped_data;
	errno = 0;
	ret->file = f;
	if ((fd = wshash_fileno(ret->file)) == -1) {
		return false;
	}
	mmapped_data= mmap(
		NULL,
		(size_t)ret->file_size + WSHASH_DAG_MAGIC_NUM_SIZE,
	...
	ret->data = (node*)(mmapped_data + WSHASH_DAG_MAGIC_NUM_SIZE);
	return true;
}

wshash_full_t wshash_full_new_internal(
	char const* dirname,
...
)
{
	...
	switch (wshash_io_prepare(dirname, seed_hash, &f, (size_t)full_size, false)) {
	case WSHASH_IO_FAIL:
		// wshash_io_prepare will do all WSHASH_CRITICAL() logging in fail case
		goto fail_free_full;
	case WSHASH_IO_MEMO_MATCH:
		if (!wshash_mmap(ret, f)) {
			WSHASH_CRITICAL("mmap failure()");
			goto fail_close_file;
		}
		return ret;
	case WSHASH_IO_MEMO_SIZE_MISMATCH:
		// if a DAG of same filename but unexpected size is found, silently force new file creation
		if (wshash_io_prepare(dirname, seed_hash, &f, (size_t)full_size, true) != WSHASH_IO_MEMO_MISMATCH) {
			WSHASH_CRITICAL("Could not recreate DAG file after finding existing DAG with unexpected size.");
			goto fail_free_full;
		}
		// fallthrough to the mismatch case here, DO NOT go through match
	case WSHASH_IO_MEMO_MISMATCH:
		if (!wshash_mmap(ret, f)) {
			WSHASH_CRITICAL("mmap failure()");
			goto fail_close_file;
		}
	...
	if (!wshash_compute_full_data(ret->data, full_size, light, callback)) {
		WSHASH_CRITICAL("Failure at computing DAG data.");
		goto fail_free_full_data;
	}

	// after the DAG has been filled then we finalize it by writting the magic number at the beginning
	if (fseek(f, 0, SEEK_SET) != 0) {
		WSHASH_CRITICAL("Could not seek to DAG file start to write magic number.");
		goto fail_free_full_data;
	}
	uint64_t const magic_num = WSHASH_DAG_MAGIC_NUM;
	if (fwrite(&magic_num, WSHASH_DAG_MAGIC_NUM_SIZE, 1, f) != 1) {
		WSHASH_CRITICAL("Could not write magic number to DAG's beginning.");
		goto fail_free_full_data;
	}
	if (fflush(f) != 0) {// make sure the magic number IS there
		WSHASH_CRITICAL("Could not flush memory mapped data to DAG file. Insufficient space?");
		goto fail_free_full_data;
	}
	return ret;
...
{
	char strbuf[256];
	if (!wshash_get_default_dirname(strbuf, 256)) {
		return NULL;
	}
	uint64_t full_size = wshash_get_datasize(light->block_number);
	wshash_h256_t seedhash = wshash_get_seedhash(light->block_number);
	return wshash_full_new_internal(strbuf, seedhash, full_size, light, callback);
}
