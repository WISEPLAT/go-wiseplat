...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/wiseplat                                                            && \
	(cd $GOPATH/src/github.com/wiseplat && git clone --depth=1 https://github.com/wiseplat/go-wiseplat) && \
  go build -v github.com/wiseplat/go-wiseplat/cmd/faucet                                              && \
...
CMD [ \
	"/faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--wshstats", "{{.Wshstats}}", "--wshport", "{{.WshPort}}", \
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Wshstats":      config.node.wshstats,
		"WshPort":       config.node.portFull,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"WshPort":       config.node.portFull,
		"WshName":       config.node.wshstats[:strings.Index(config.node.wshstats, ":")],
		"GitHubUser":    config.githubUser,
		"GitHubToken":   config.githubToken,
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
...
func (info *faucetInfos) String() string {
	return fmt.Sprintf("host=%s, api=%d, wsh=%d, amount=%d, minutes=%d, tiers=%d, github=%s, captcha=%v, wshstats=%s", info.host, info.port, info.node.portFull, info.amount, info.minutes, info.tiers, info.githubUser, info.captchaToken != "", info.node.wshstats)
}
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			portFull: infos.portmap[infos.envvars["WSH_PORT"]+"/tcp"],
			wshstats: infos.envvars["WSH_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
