...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Wises to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.wshstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.wshstats = w.readString() + ":" + w.conf.wshstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.wshstats)
		infos.node.wshstats = w.readDefaultString(infos.node.wshstats) + ":" + w.conf.wshstats
	}
