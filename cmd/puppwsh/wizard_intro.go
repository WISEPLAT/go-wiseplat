...
// makeWizard creates and returns a new puppwsh wizard.
...
// setting up a new or managing an existing Wiseplat private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppwsh, your Wiseplat private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Wiseplat network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and wshstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppwsh uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Wiseplat network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppwsh", w.network)
