...
func (w *wizard) networkStats(tips bool) {
	...
	for server, pubkey := range w.conf.Servers {
		...
		logger.Debug("Checking for wshstats availability")
		if infos, err := checkWshstats(client, w.network); err != nil {
			if err != ErrServiceUnknown {
				services["wshstats"] = err.Error()
			}
		} else {
			services["wshstats"] = infos.String()
			protips.wshstats = infos.config
		}
	...
	if protips.wshstats != "" {
		w.conf.wshstats = protips.wshstats
	}
...
func (p *protips) print(network string) {
	...
	fullinit, lightinit := "", ""
	if p.genesis != "" {
		fullinit = fmt.Sprintf("gwsh --datadir=$HOME/.%s init %s.json && ", network, network)
		lightinit = fmt.Sprintf("gwsh --datadir=$HOME/.%s --light init %s.json && ", network, network)
	}
	// If an wshstats server is available, add the wshstats flag
	statsflag := ""
	if p.wshstats != "" {
		if strings.Contains(p.wshstats, " ") {
			statsflag = fmt.Sprintf(` --wshstats="yournode:%s"`, p.wshstats)
		} else {
			statsflag = fmt.Sprintf(` --wshstats=yournode:%s`, p.wshstats)
		}
	}
	...
	tips = append(tips, fmt.Sprintf("%sgwsh --networkid=%d --datadir=$HOME/.%s --cache=1024%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a full node with recent data only")
	tips = append(tips, fmt.Sprintf("%sgwsh --networkid=%d --datadir=$HOME/.%s --cache=512 --fast%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a light node with on demand retrievals")
	tips = append(tips, fmt.Sprintf("%sgwsh --networkid=%d --datadir=$HOME/.%s --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))

	tasks = append(tasks, "Run an embedded node with constrained memory")
	tips = append(tips, fmt.Sprintf("%sgwsh --networkid=%d --datadir=$HOME/.%s --cache=32 --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))
