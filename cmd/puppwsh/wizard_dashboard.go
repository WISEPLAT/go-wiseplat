...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"wshstats", "explorer", "wallet", "faucet"} {
	...
	// If we have wshstats running, ask whether to make the secret public or not
	var wshstats bool
	if w.conf.wshstats != "" {
		fmt.Println()
		fmt.Println("Include wshstats secret on dashboard (y/n)? (default = yes)")
		wshstats = w.readDefaultString("y") == "y"
	}
	// Try to deploy the dashboard container on the host
	if out, err := deployDashboard(client, w.network, infos.port, infos.host, listing, &w.conf, wshstats); err != nil {
