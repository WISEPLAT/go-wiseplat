...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.wshstats == "" {
		log.Error("No wshstats server configured")
		return
	}
	...
	if infos.wshstats == "" {
		...
		infos.wshstats = w.readString() + ":" + w.conf.wshstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.wshstats)
		infos.wshstats = w.readDefaultString(infos.wshstats) + ":" + w.conf.wshstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.genesis.Config.Wshash != nil {
			// Wshash based miners only need an wisebase to mine against
			fmt.Println()
			if infos.wisebase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.wisebase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.wisebase)
				infos.wisebase = w.readDefaultAddress(common.HexToAddress(infos.wisebase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy Wiseplat node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
