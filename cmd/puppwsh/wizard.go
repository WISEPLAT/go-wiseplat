...
// config contains all the configurations needed by puppwsh that should be saved
// between sessions.
type config struct {
	...
	wshstats  string        // Wshstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppwsh configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Wiseplat services known to be running on servers
...
// converts it to an Wiseplat address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by wshstats, which compares IPs textually, not by value.
