...
// faucet is a Wise faucet backed by a light client.
package main
...
var (
	...
	apiPortFlag = flag.Int("apiport", 8787, "Listener port for the HTTP API connection")
	wshPortFlag = flag.Int("wshport", 30373, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Wiseplat protocol")
	statsFlag   = flag.String("wshstats", "", "Wshstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Wises to pay out per user request")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for Wiseplat and the faucet")
)

var (
	wise = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Wises", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *wshPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Wiseplat address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Wiseplat light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Wiseplat protocol stack
	client *wshclient.Client   // Client connection to the Wiseplat chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Wiseplat light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := wsh.DefaultConfig
	...
	// Assemble the wshstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightWiseplat
			ctx.Service(&serv)
			return wshstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := wshclient.NewClient(api)
...
// close terminates the Wiseplat connection and tears down the faucet.
...
// apiHandler handles requests for Wise grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, wise),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Wiseplat address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/wiseplat/go-wiseplat/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), wise)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance = new(big.Int).Div(balance, wise)
...
// the username, avatar URL and Wiseplat address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Wiseplat addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Wiseplat address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Wiseplat address and profile URL.
...
// returning the username, avatar URL and Wiseplat address to fund on success.
