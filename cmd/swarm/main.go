...
import (
	...
	"github.com/wiseplat/go-wiseplat/cmd/utils"
	"github.com/wiseplat/go-wiseplat/common"
	"github.com/wiseplat/go-wiseplat/console"
	"github.com/wiseplat/go-wiseplat/contracts/ens"
	"github.com/wiseplat/go-wiseplat/crypto"
	"github.com/wiseplat/go-wiseplat/wshclient"
	"github.com/wiseplat/go-wiseplat/internal/debug"
	...
	"github.com/wiseplat/go-wiseplat/swarm"
	bzzapi "github.com/wiseplat/go-wiseplat/swarm/api"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:  "swap-api",
		Usage: "URL of the Wiseplat API provider to use to settle SWAP payments",
	}
	...
	EnsAPIFlag = cli.StringFlag{
		Name:  "ens-api",
		Usage: "URL of the Wiseplat API provider to use for ENS record lookups",
		Value: node.DefaultIPCEndpoint("gwsh"),
	}
	...
	DeprecatedWshAPIFlag = cli.StringFlag{
		Name:  "wshapi",
...
// This init function sets defaults so cmd/swarm can run alongside gwsh.
func init() {
...
var app = utils.NewApp(gitCommit, "Wiseplat Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The go-wiseplat Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.wiseplat/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.wiseplat/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.wiseplat/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.wiseplat/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	// exit if the deprecated --wshapi flag is set
	if ctx.GlobalString(DeprecatedWshAPIFlag.Name) != "" {
		utils.Fatalf("--wshapi is no longer a valid command line flag, please use --ens-api and/or --swap-api.")
	}
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := wshclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
...
func registerBzzService(ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		...
		if ensapi != "" {
			...
			ensClient = wshclient.NewClient(client)
