...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gwsh.LightScryptN, Gwsh.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.wiseplat", new NodeConfig());
			node.start();
			...
			WiseplatClient ec = node.getWiseplatClient();
	...
	// signatures too. Regression test for go-wiseplat issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gwsh-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/wiseplat/gwshtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gwsh and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.wiseplat", "github.com/wiseplat/go-wiseplat/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gwsh.aar"), "gwsh.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "wiseplat", "gwshtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
