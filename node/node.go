...
func (n *Node) OpenDatabase(name string, cache, handles int) (wshdb.Database, error) {
	if n.config.DataDir == "" {
		return wshdb.NewMemDatabase()
	}
	return wshdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
