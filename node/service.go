...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (wshdb.Database, error) {
	if ctx.config.DataDir == "" {
		return wshdb.NewMemDatabase()
	}
	db, err := wshdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
