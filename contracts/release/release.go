...
type Config struct {
	Oracle common.Address // Wiseplat address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Wiseplat service dependency to access the blockchain
	var apiBackend wshapi.Backend
	var wiseplat *wsh.Wiseplat
	if err := ctx.Service(&wiseplat); err == nil {
		apiBackend = wiseplat.ApiBackend
	} else {
		var wiseplat *les.LightWiseplat
		if err := ctx.Service(&wiseplat); err == nil {
			apiBackend = wiseplat.ApiBackend
		} else {
			return nil, err
	...
	contract, err := NewReleaseOracle(config.Oracle, wsh.NewContractBackend(apiBackend))
	if err != nil {
		return nil, err
	}
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/wiseplat/go-wiseplat/releases for new releases")
