...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout wsh_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for WshSubscribe requests
}
...
// WshSubscribe registers a subscripion under the "wsh" namespace.
func (c *Client) WshSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "wsh", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. WshSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through WshSubscribe.
