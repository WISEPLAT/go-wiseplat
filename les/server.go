...
import (
	...
	"github.com/wiseplat/go-wiseplat/les/flowcontrol"
...
func NewLesServer(wsh *wsh.Wiseplat, config *wsh.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(wsh.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, wsh.EventMux(), wsh.Engine(), newPeerSet(), wsh.BlockChain(), wsh.TxPool(), wsh.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range ServerProtocolVersions {
		lesTopics[i] = lesTopic(wsh.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(wsh.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(wsh.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(wsh.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(wsh.ChainDb())
	return srv, nil
}
...
func newCostStats(db wshdb.Database) *requestCostStats {
