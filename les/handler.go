...
const (
	...
	wshVersion = 63 // equivalent wsh version for the downloader
...
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, protocolVersions []uint, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb wshdb.Database, odr *LesOdr, txrelay *LesTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Wiseplat protocol")
	...
	log.Info("Light Wiseplat protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Wiseplat peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Wiseplat message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Wiseplat message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	case GetHeaderProofsMsg:
		...
		trieDb := wshdb.NewTable(pm.chainDb, light.ChtTablePrefix)
		for _, req := range req.Reqs {
			if bytes >= softResponseLimit {
				break
			}
	...
	case GetHelperTrieProofsMsg:
		...
		for _, req := range req.Reqs {
			...
			if tr == nil || req.HelperTrieType != lastType || req.TrieIdx != lastIdx {
				...
				if root != (common.Hash{}) {
					if t, err := trie.New(root, wshdb.NewTable(pm.chainDb, prefix)); err == nil {
						tr = t
					}
				}
...
func (self *ProtocolManager) NodeInfo() *wsh.WshNodeInfo {
	return &wsh.WshNodeInfo{
...
func (d *downloaderPeerNotify) registerPeer(p *peer) {
	...
	pm.downloader.RegisterLightPeer(p.id, wshVersion, pc)
}
