...
import (
	...
	"github.com/wiseplat/go-wiseplat/internal/jsre"
	"github.com/wiseplat/go-wiseplat/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Wiseplat requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jwsh", struct{}{})

	jwshObj, _ := c.jsre.Get("jwsh")
	jwshObj.Object().Set("send", bridge.Send)
	jwshObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jwsh);"); err != nil {
	...
	flatten := "var wsh = web3.wsh; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = wsh.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jwsh.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jwsh.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jwsh.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jwsh.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jwsh.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines wsh.getBalance(wsh.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gwsh instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gwsh metadata
	fmt.Fprintf(c.printer, "Welcome to the Gwsh JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + wsh.coinbase);
		console.log("at block: " + wsh.blockNumber + " (" + new Date(1000 * wsh.getBlock(wsh.blockNumber).timestamp) + ")");
