...
func newTester(t *testing.T, confOverride func(*wsh.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Wiseplat service within
	...
	wshConf := &wsh.Config{
		...
		Wisebase: common.HexToAddress(testAddress),
		PowTest:   true,
	}
	if confOverride != nil {
		confOverride(wshConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return wsh.New(ctx, wshConf) }); err != nil {
		t.Fatalf("failed to register Wiseplat protocol: %v", err)
	}
