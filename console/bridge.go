...
import (
	...
	"github.com/wiseplat/go-wiseplat/accounts/usbwallet"
...
// RPC method (saved in jwsh.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call otto.FunctionCall) (response otto.Value) {
	...
	ret, err := call.Otto.Call("jwsh.newAccount", nil, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
func (b *bridge) OpenWallet(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jwsh.openWallet", nil, wallet, passwd)
	if err == nil {
		return val
	}
	...
	if val, err = call.Otto.Call("jwsh.openWallet", nil, wallet, passwd); err != nil {
		throwJSException(err.Error())
	}
	return val
}
...
// original RPC method (saved in jwsh.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jwsh.unlockAccount", nil, account, passwd, duration)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
// jwsh.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call otto.FunctionCall) (response otto.Value) {
	...
	// Send the request to the backend and return
	val, err := call.Otto.Call("jwsh.sign", nil, message, account, passwd)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) SleepBlocks(call otto.FunctionCall) (response otto.Value) {
	...
	blockNumber := func() int64 {
		result, err := call.Otto.Run("wsh.blockNumber")
		if err != nil {
			throwJSException(err.Error())
		}
