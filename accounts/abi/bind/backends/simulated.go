...
type SimulatedBackend struct {
	database   wshdb.Database   // In memory database to store our testing data
	blockchain *core.BlockChain // Wiseplat blockchain to handle the consensus
...
func NewSimulatedBackend(alloc core.GenesisAlloc) *SimulatedBackend {
	database, _ := wshdb.NewMemDatabase()
	genesis := core.Genesis{Config: params.AllWshashProtocolChanges, Alloc: alloc}
	genesis.MustCommit(database)
	blockchain, _ := core.NewBlockChain(database, genesis.Config, wshash.NewFaker(), vm.Config{})
...
func (b *SimulatedBackend) CallContract(ctx context.Context, call wiseplat.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call wiseplat.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call wiseplat.CallMsg) (*big.Int, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call wiseplat.CallMsg, block *types.Block, statedb *state.StateDB) ([]byte, *big.Int, bool, error) {
