...
import "github.com/wiseplat/go-wiseplat/accounts/abi"
...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		// Deploy{{.Type}} deploys a new Wiseplat contract, binding an instance of {{.Type}} to it.
	...
	// {{.Type}} is an auto generated Go binding around an Wiseplat contract.
	type {{.Type}} struct {
	  {{.Type}}Caller     // Read-only binding to the contract
	  {{.Type}}Transactor // Write-only binding to the contract
	}

	// {{.Type}}Caller is an auto generated read-only Go binding around an Wiseplat contract.
	type {{.Type}}Caller struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Transactor is an auto generated write-only Go binding around an Wiseplat contract.
	type {{.Type}}Transactor struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Session is an auto generated Go binding around an Wiseplat contract,
	...
	// {{.Type}}CallerSession is an auto generated read-only Go binding around an Wiseplat contract,
	...
	// {{.Type}}TransactorSession is an auto generated write-only Go binding around an Wiseplat contract,
	...
	// {{.Type}}Raw is an auto generated low-level Go binding around an Wiseplat contract.
	type {{.Type}}Raw struct {
	  Contract *{{.Type}} // Generic contract binding to access the raw methods on
	}

	// {{.Type}}CallerRaw is an auto generated low-level read-only Go binding around an Wiseplat contract.
	type {{.Type}}CallerRaw struct {
		Contract *{{.Type}}Caller // Generic read-only contract binding to access the raw methods on
	}

	// {{.Type}}TransactorRaw is an auto generated low-level write-only Go binding around an Wiseplat contract.
...
import org.wiseplat.gwsh.*;
import org.wiseplat.gwsh.internal.*;
...
{{range $contract := .Contracts}}
	public class {{.Type}} {
		...
		{{if .InputBin}}
			...
			// deploy deploys a new Wiseplat contract, binding an instance of {{.Type}} to it.
			public static {{.Type}} deploy(TransactOpts auth, WiseplatClient client{{range .Constructor.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = Gwsh.newInterfaces({{(len .Constructor.Inputs)}});
				{{range $index, $element := .Constructor.Inputs}}
				  args.set({{$index}}, Gwsh.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}
				return new {{.Type}}(Gwsh.deployContract(auth, ABI, BYTECODE, client, args));
			}
		...
		// Wiseplat address where this contract is located at.
		public final Address Address;

		// Wiseplat transaction in which this contract was deployed (if known!).
		...
		public {{.Type}}(Address address, WiseplatClient client) throws Exception {
			this(Gwsh.bindContract(address, ABI, client));
		}
		...
		{{range .Calls}}
			...
			public {{if gt (len .Normalized.Outputs) 1}}{{capitalise .Normalized.Name}}Results{{else}}{{range .Normalized.Outputs}}{{bindtype .Type}}{{end}}{{end}} {{.Normalized.Name}}(CallOpts opts{{range .Normalized.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = Gwsh.newInterfaces({{(len .Normalized.Inputs)}});
				{{range $index, $item := .Normalized.Inputs}}args.set({{$index}}, Gwsh.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}

				Interfaces results = Gwsh.newInterfaces({{(len .Normalized.Outputs)}});
				{{range $index, $item := .Normalized.Outputs}}Interface result{{$index}} = Gwsh.newInterface(); result{{$index}}.setDefault{{namedtype (bindtype .Type) .Type}}(); results.set({{$index}}, result{{$index}});
				{{end}}

				if (opts == null) {
					opts = Gwsh.newCallOpts();
				}
