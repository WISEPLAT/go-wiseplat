...
import (
	...
	"github.com/wiseplat/go-wiseplat/accounts/usbwallet/internal/trezor"
...
// or whether the Wiseplat app was not started on it.
...
// Wiseplat address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.WiseplatAddress)
	if _, err := w.trezorExchange(&trezor.WiseplatGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.WiseplatTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.WiseplatTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
