...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Wiseplat
...
type wallet struct {
	...
	deriveChain    wiseplat.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Wiseplat account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain wiseplat.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
