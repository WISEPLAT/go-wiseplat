...
// PublicWiseplatAPI provides an API to access Wiseplat related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicWiseplatAPI struct {
	b Backend
}

// NewPublicWiseplatAPI creates a new Wiseplat protocol API.
func NewPublicWiseplatAPI(b Backend) *PublicWiseplatAPI {
	return &PublicWiseplatAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicWiseplatAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Wiseplat protocol version this node supports
func (s *PublicWiseplatAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicWiseplatAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Wiseplat Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Wiseplat Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Wiseplat ECDSA signature for:
// keccack256("\x19Wiseplat Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/wiseplat/go-wiseplat/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with wsh_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Wiseplat Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/wiseplat/go-wiseplat/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Wiseplat signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Wiseplat blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Wiseplat blockchain API.
...
//
// https://github.com/wiseplat/wiki/wiki/JSON-RPC#wsh_sign
...
// PublicDebugAPI is the collection of Wiseplat APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", wshash.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of Wiseplat APIs exposed over the private
// debugging endpoint.
...
// Version returns the current wiseplat protocol version.
