...
#
# The cluster contains a bootnode, a gwsh node and multiple swarm nodes, with
...
# static gwsh configuration
GWSH_IP="192.168.33.3"
GWSH_RPC_PORT="8747"
GWSH_RPC_URL="http://${GWSH_IP}:${GWSH_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gwsh and
# swarm nodes
start_bootnode() {
...
# start_gwsh_node starts a gwsh node with --datadir pointing at <base-dir>/gwsh
# and a single, unlocked account with password "gwsh"
start_gwsh_node() {
  local dir="${base_dir}/gwsh"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gwsh account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GWSH_IP}"
    --rpcport   "${GWSH_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gwsh" "${GWSH_IP}" "$(which gwsh)" ${args[@]}
}
...
create_account() {
  ...
  gwsh --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
